...
def parse_with_pvp_class(class_ = "PVP_Project"):
    ...
    for data in datas:
        ...
        for num_exp, experiment in enumerate(data['experiments'].values()):
            ...
            if (EXPORT_FLAG):
                ...
                export_path = "/home/quentin/Cours/ANDROIDE_Project_HCI_Fitts2.0/data_extractor/DATA/users/"+str(npart)
                ...
                export_path = "/home/quentin/Cours/ANDROIDE_Project_HCI_Fitts2.0/data_extractor/DATA/users/"+str(npart)+'/'+class_+'/std_profiles/'
        ...
        if (WRITE_FLAG):
            ...
            export_path = "/home/quentin/Cours/ANDROIDE_Project_HCI_Fitts2.0/data_extractor/DATA/users/"+str(npart)+'/'+class_+"/data/"
            if not os.path.exists(export_path):
                os.makedirs(export_path)
        ...
        if (SHOW_FLAG or EXPORT_FLAG) :
            ...
            pvp_container = PVP_container(npart, [k[0] for k in fitts_controller_container])
            pvp_container.plot_all_pvps(axs[0,0],[k[1] for k in fitts_controller_container])
            axs[0,0].title.set_text('fitts controller')
            
            pvp_container = PVP_container(npart, [k[0] for k in fitts_mouse_container])
            pvp_container.plot_all_pvps(axs[0,1],[k[1] for k in fitts_mouse_container])
            axs[0,1].title.set_text('fitts mouse')
            
            pvp_container = PVP_container(npart, [k[0] for k in fitts_touchpad_container])
            pvp_container.plot_all_pvps(axs[0,2],[k[1] for k in fitts_touchpad_container])
            axs[0,2].title.set_text('fitts touchpad')
            
            pvp_container = PVP_container(npart, [k[0] for k in pvp_controller_container])
            pvp_container.plot_all_pvps(axs[1,0],[k[1] for k in pvp_controller_container ])
            ...
            pvp_container.plot_all_pvps(axs[1,2],[k[1] for k in pvp_touchpad_container ])
        ...
        ndata += 1
        if (EXPORT_FLAG):
            export_path = "/home/quentin/Cours/ANDROIDE_Project_HCI_Fitts2.0/data_extractor/DATA/users/"+str(npart)+"/"+class_+'/'
            if not os.path.exists(export_path):
                os.makedirs(export_path)
    ...
    if (WRITE_FLAG):
        export_path = "/home/quentin/Cours/ANDROIDE_Project_HCI_Fitts2.0/data_extractor/DATA/export/"
        export_path += class_+'/'
        if not os.path.exists(export_path):
            os.makedirs(export_path)
