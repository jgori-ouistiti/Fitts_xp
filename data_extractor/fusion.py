...
def fusion_dataframe(data_path = "/home/quentin/Cours/ANDROIDE_Project_HCI_Fitts2.0/data_extractor/DATA/users/", export_path = "/home/quentin/Cours/ANDROIDE_Project_HCI_Fitts2.0/data_extractor/DATA/export/"):
    ...
    for user_id in next(os.walk(data_path))[1]:
        ...
        for class_ in next(os.walk(data_path+user_id+'/'))[1]:
            ...
            if is_empty_stats:
                try:
                    dataframes_stats = pandas.read_csv(path+'/data/pvp_stats.csv', index_col = 0)
                ...
                try:
                    ...
                    dataframes_stats = pandas.concat([dataframes_stats, user_df], ignore_index = True)
                except:
                    print("Cannot read :" +path+'/data/pvp_stats.csv')
            ...
            if is_empty_profile:
                try:
                    dataframes_profile = pandas.read_csv(path+'/data/pvp_profile.csv', index_col = 0)
                    is_empty_profile = False
                except:
                    print("Cannot read :" +path+'/data/pvp_profile.csv')
            else:
                try:
                    user_df = pandas.read_csv(path+'/data/pvp_profile.csv', index_col = 0)
                    dataframes_profile = pandas.concat([dataframes_profile, user_df], ignore_index = True)
                except:
                    print("Cannot read :" +path+'/data/pvp_stats.csv')
