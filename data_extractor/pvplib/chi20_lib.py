...
def interp_filt(t,x, *args, _filter = {"filtername" : "kaiser", 'fc' : 10, 'rdb': 10, 'width': 5}, deriv = 2, zero_time = True):
...
class FITTS_single:
    ...
    def compute_end_movement(self, func, *args):
        ...
        self.cond_indx = numpy.ma.masked_array(conditions_all, mask = numpy.isnan(conditions_all))
    ...
    def make_table_summary(self, latex_path):
        ...
        tex_add_row_tabular(latex_path, ["", "N Obs", "df$_r$/df$_m$", "$R^2$/R$^2_{adj}$", "F/p(F)", "log-llh", "AIC", "BIC"])
        tex_add_row_tabular(latex_path, ["", dic['No. Observations'], str(dic['df Residuals'])+"/"+str(dic['df Model']), str(dic['r-squared'])+"/"+str(dic['Adj. r-squared']), str(dic['F-statistic'])+"/"+str(dic['Prob (F-statistic)']), dic['Log-likelihood'], dic['AIC'], dic['BIC']], tol = 3)
        tex_add_custom_line(latex_path, "midrule")
        tex_add_row_tabular(latex_path, ["", "Estimate", "Std Error", "$t$", "p(t)", "CI:", "[0.025", "0.975]"], tol = 3)
        tex_add_row_tabular(latex_path, ["Intercept", dic['Intercept-a'], dic['Std Error-a'], dic['t-a'], dic['P>t-a'], "", float(dic['CI-a'][0]), float(dic['CI-a'][1])], tol = 3)
        tex_add_row_tabular(latex_path, ["Slope", dic['Slope-b'], dic['Std Error-b'], dic['t-b'], dic['P>t-b'], "", float(dic['CI-b'][0]), float(dic['CI-b'][1])], tol = 3)
    ...
    def get_summary(self):
        ...
        result= self.result
        dic = { 'No. Observations': int(result.nobs),
...
class PVP_container:
    ...
    def make_data_frame(self):
        dataframe = pandas.DataFrame(columns=['Participant', 'D', 'ID', 'W', 'tau', 'Dtau','sigma0', 'Omega', 'C', 'rsq'])
        for i,container in enumerate(self.container):
            row = [self.participant, *[float(u) for u in container.condition[1:]], container.tau, container.Dtau, container.sigma_0, container.pvp_params[2], -container.pvp_params[1], container.secondphase_fit.rsquared]
            dataframe.loc[i] = row
        return dataframe

    def dataframe_to_latex(self, latex_path):
        df = self.dataframe
        tex_begin_tabular(latex_path, 9, position_str = 'rrrrrrrrr', caption = 'Summary of PVP statistics for Participant {}'.format(self.participant))
        latex_columns = ['D\,(mm)', 'ID', 'W\,(mm)', '$\\tau$\,(s)', 'D$_{\\tau}$\,(eq. mm)',  '$\\sigma_0$\,(eq. mm)', '$\Omega$\,(s)', 'C\,(bit/s)', '$R^2$']
...
class PVP:
    ...
    def add_2D_traj_raw(self,x,y,t, *args, correct_start = 'yes'):
        ...
        container, TS = interp_filt(numpy.array(t),numpy.array(d),1e-2, _filter = {"filtername" : "kaiser", 'fc' : 8, 'rdb': 10, 'width': 5}, deriv = 1, zero_time = True)
        self.TS = TS
    ...
    def find_start(self, container, method, *args):
        ...
        container = numpy.concatenate((time[indx:].reshape(1,-1), traj[indx:].reshape(1,-1)), axis = 0)
    ...
    def extend(self, _traj, *args):
        ...
        if self.traj.shape[0] == 0:
            ...
            while self.traj.shape[1] < _traj.shape[1]:
                self.traj = numpy.concatenate((self.traj, self.traj[:,-1].reshape((-1,1))), axis = 1)
            while self.traj.shape[1] > _traj.shape[1]:
                _traj = numpy.append(_traj,_traj[0,-1]).reshape(1,-1)
    ...
    def fit_profiles(self, *args):
        ...
        res = opti.basinhopping(  func = monospline,
                        x0 = theta0,
                        niter = n_iter,
                        minimizer_kwargs = {'method': "Nelder-Mead", 'args': (x,y),  'options' : {'maxiter' : 1000, 'disp' : 0}}
        )
        ...
        _yy = [a + b*i*self.TS for i in range(0,c0)] + [a + c*b for i in range(c0,len(y[indx_tau:]))]
...
class PVP_Reverse(PVP):
    ...
    def add_2D_traj_raw(self,x,y,t, *args, correct_start = 'yes'):
        ...
        container, TS = interp_filt(numpy.array(t),numpy.array(d), _filter = {"filtername" : "kaiser", 'fc' : 8, 'rdb': 10, 'width': 5}, deriv = 1, zero_time = True)
        self.TS = TS
        if correct_start == 'yes':
...
class PVP_Project(PVP):
    ...
    def add_2D_traj_raw_x(self,x,y,t, *args, correct_start = 'yes'):
        ...
        container, TS = interp_filt(numpy.array(t),numpy.array(_proj), _filter = {"filtername" : "kaiser", 'fc' : 8, 'rdb': 10, 'width': 5}, deriv = 1, zero_time = True)
        self.TS = TS
        if correct_start == 'yes':
            container,_ = self.find_start(container, "threshold", 3)
        else:
...
class PVP_Centroid(PVP):
    ...
    def add_2D_traj_raw(self,x,y,t, *args, correct_start = 'yes'):
        ...
        containerx, TS = interp_filt(numpy.array(t),numpy.array(_projx), TS, _filter = {"filtername" : "kaiser", 'fc' : 8, 'rdb': 10, 'width': 5}, deriv = 1, zero_time = True)
        containery, TS = interp_filt(numpy.array(t),numpy.array(_projy), TS, _filter = {"filtername" : "kaiser", 'fc' : 8, 'rdb': 10, 'width': 5}, deriv = 1, zero_time = True)
        self.TS = TS
        if correct_start == 'yes':
    ...
    def extend(self,trajs, _traj, *args):
        ...
        if trajs.shape[0] == 0:
            ...
            while trajs.shape[1] < _traj.shape[1]:
                trajs = numpy.concatenate((trajs, trajs[:,-1].reshape((-1,1))), axis = 1)
            while trajs.shape[1] > _traj.shape[1]:
                _traj = numpy.append(_traj,_traj[0,-1]).reshape(1,-1)
    ...
    def compute_profiles(self):
        ...
        self.std_prof = numpy.sqrt(numpy.square(self.std_prof_x) + numpy.square(self.std_prof_y))
    ...
    def remove_outliers(self, sigma_away, *args):
        ...
        while k < len(self.traj_x):
            ...
            cpt = 0
            for ns, sample in enumerate(traj):
                if abs(sample -self.mean_prof_x[ns]) > sigma_away*self.std_prof_x[ns]:
                    cpt += 1
                if cpt > 5:
        ...
        while k < len(self.traj_y):
            ...
            cpt = 0
            for ns, sample in enumerate(traj):
                if abs(sample -self.mean_prof_y[ns]) > sigma_away*self.std_prof_y[ns]:
                    cpt += 1
                if cpt > 5:
                    self.traj_x = numpy.delete(self.traj_x, k, axis = 0)
                    self.traj_y = numpy.delete(self.traj_y, k, axis = 0)
...
def multitraj_get_starts(timestamps, position, speed, threshold = 1e-2, multitraj_type = 'both'):

    def get_start(mov_indx, timestamps, nposition, nspeed, threshold = 1e-2, multitraj_type = 'both'):
        idx = mov_indx
    ...
    for mov_indx in _movs:
        if multitraj_type == 'ascending' and _spd[mov_indx] > 0:
            output.append(get_start(mov_indx, timestamps, position, speed, threshold = 1e-2, multitraj_type = multitraj_type))
...
def get_stop(timestamps, pos, speed, _thresh = 1e-2):
    ...
    # plt.show()
    # plt.close()
    return first_zero, last_dwell_begin, last_dwell_mid, last_dwell_end, first_dwell_mid, mid_longest_dwell, plateaux
