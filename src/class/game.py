...
class Game:
    def __init__(
        self, width, height, bg_color=Colors.WHITE, title="TEST_CIBLES", fullscreen=True
    ):
        ...
        self.listener = (
            dict()
        )  # Key = Listener object, value = boolean (True if listening, False if not listening)
        self.drawables = (
            dict()
        )  # Key = Drawable object, value = boolean (True if is on screen, False if not )
        ...
        self.inputBoxAvis = textInputBox.TextInputBox(
            (self.width / 2 - 200, self.height * 2 / 3 + 100),
            self.width,
            self.height,
            width_texte=400,
            height_texte=100,
            bg_color_text=Colors.LIGHT_BLUE,
        )
    ...
    def write_screen(self, text, color, pos, maxSize=None):
        ...
        words = [
            mot.split(" ") for mot in text.splitlines()
    ...
    def addListenerDrawable(self, ld):
        ...
        if hasattr(ld, "__len__"):
            for ld_item in ld:
                if not isinstance(ld_item, Drawable) or not isinstance(
                    ld_item, Listener
                ):
        ...
        if not isinstance(ld, Drawable) and not isinstance(ld, Listener):
            raise Exception(
                "ld is not a listener and not drawable, but is a "
                + ld.__class__.__name__
            )
    ...
    def endGame(self):
        ...
        self.refreshScreen()
        self.write_box(
            "GAME OVER", Colors.BLACK, (self.width / 2, self.height / 3 - 50)
        )
        self.write_box(
    ...
    def play(self, mode="", listTarget=[], showTime=True, displayConsolNbOfTarget=True):
        ...
        self.cursor_position.append(
            ("target_pos :", (self.active_target.x, self.active_target.y))
        )
        while self.running:
            ...
            if showTime and mode != "experienceMulti":
                # Display Timer
                self.write_screen(
                    "Time : " + "{:.1f}".format(self.barTime.timer),
                    Colors.BLACK,
                    self.barTime.posText,
                )
                pygame.display.update()
    ...
    def chooseMode(self):
        ...
        button1 = Button(
            (int(self.width / 2 - 500), int(self.height / 2 + 30)),
            1,
            300,
            60,
            (200, 50, 50),
        ...
        button3 = Button(
            (int(self.width / 2 + 200), int(self.height / 2 + 30)),
            3,
            300,
            60,
            (200, 50, 50),
        ...
        self.write_screen(
            "Choose Your Mode",
            Colors.BLACK,
            (self.width / 2 - 180, self.height / 2 - 30),
        )
        self.running = True
        while self.running:
            pygame.display.update()
    ...
    def quickMode(
        self, mode="", listTarget=[], showTime=True, displayConsolNbOfTarget=True
    ):
    ...
    def reactiveMode(self, ID=3, A=40, p=0.25, color=Colors.BLACK, nb_trials=10):
        ...
        L_targets = make_2D_distractor_target_list(
            (self.width, self.height),
            (int(self.width / 2), int(self.height / 2)),
            ID,
            A,
