...
class GameExperiment(Game):
    ...
    def __init__(
        ...
        cursorImage="class/cursor/cursor1.png",
        title="TEST CIBLES",
        fullscreen=True,
    ):
        ...
        self.experiments_data = (
            dict()
        )  # User's data collected after ends of each experiments
        ...
        self.joy_is_init = (
            False  # boolean used to know if pygame.joystick has been initiated or not
        )
        self.language = (
            "en"  # default = en, can be changed with chooseLanguage() screen
        )
        self.cursor = cursor
        self.joystick = None
        ...
        if hasattr(experiments, "__len__"):
            for experiment in experiments:
                if not isinstance(experiment, Experiment):
                    raise Exception(
                        "excepted Experiment but got " + experiment.__class__.__name__
                    )
            self.experiments = experiments  # list of experiments objects
        else:
            if not isinstance(experiments, Experiment):
                raise Exception(
                    "excepted Experiment or list(Experiment) but got "
                    + experiments.__class__.__name__
                )
            self.experiments = [experiments]  # list of experiments objects
    ...
    def menu(self, menu_title, current_mode="play", data=None, noPause=False):
        if menu_title == "play":
            ...
            pygame.time.set_timer(
                pygame.USEREVENT, 10
            )  # Active pygame.USEREVENT toute les 10ms
    ...
    def play(self, mode="", listTarget=[], displayConsolNbOfTarget=True):
        ...
        try:
            ...
            print(
                "Error in experiment : "
                + self.experiments[self.activeExperiment].data["exp_name"]
                + "\nError during experiment, cought a pygame parachute, skipping to next experiment."
            )
            self.removeListenerDrawable(self.listTarget)
            return self.menu(
                "endExperiment",
                self.experiments[self.activeExperiment].data,
                self.experiments[self.activeExperiment].noPause,
            )
    ...
    def chooseDevice(self):
        ...
        devices = (
            []
        )  # Putting devices in an array list to add more languages in the future
        buttons = []
    ...
    def chooseMode(self):
        ...
        if self.experiments[self.activeExperiment].input_device != None:
            buttons = []
            self.experiments_data["input_device"] = self.experiments[
                self.activeExperiment
            ].input_device
        ...
        if self.language == "en":
            button1 = Button(
                (int(self.width / 2 + 160), int(self.height / 2 - 30)),
                1,
                300,
                60,
                (200, 50, 50),
            ...
            button1 = Button(
                (int(self.width / 2 + 150), int(self.height / 2 - 30)),
                1,
                340,
                60,
                (200, 50, 50),
        ...
        if self.language == "en":
            if self.experiments[self.activeExperiment].input_device != None:
                text = (
                    ...
                    + "on the red targets.\n\n"
                    + "WARNING : The first experiment require a "
                    + self.experiments[self.activeExperiment].input_device
                    + "\n\n"
            ...
            if self.experiments[self.activeExperiment].input_device != None:
                text = (
                    ...
                    + "Vous allez avoir plusieurs petites expériences. Il vous faudra cliquer "
                    + "sur les cibles rouges avec diverses stratégies (rapide, précis, entre deux)\n\n"
                    + "ATTENTION : La première expérience utilisera un(e) "
                    + self.experiments[self.activeExperiment].input_device
                    + "\n\n"
        ...
        self.write_screen(
            text,
            Colors.BLACK,
            (self.width / 8, self.height / 2 - self.height / 4),
            maxSize=(720, 300),
        )
        running = True
        ...
        if self.activeExperiment in self.listTimerPause.keys():
            ...
            while running and timer >= 0:
                ...
                if self.language == "en":
                    ...
                    self.write_box(
                        " Continuing in " + "{:.1f}".format(timer) + " seconds",
                        Colors.BLACK,
                        (self.width / 2, self.height / 2 + 30),
                    )
                elif self.language == "fr":
                    self.write_box(
                        " Expérience temporairement en pause ! ",
                        Colors.BLACK,
                        (self.width / 2, self.height / 2 - 30),
                    )
                    self.write_box(
                        " L'expérience reprend dans "
                        + "{:.1f}".format(timer)
                        + " secondes",
                        Colors.BLACK,
                        (self.width / 2, self.height / 2 + 30),
    ...
    def endExperimentScreen(self, noPause=False):
        ...
        while running and not noPause:
            ...
            if self.language == "en":
                ...
                if self.experiments[self.activeExperiment].input_device != None:
                    self.write_box(
                        " WARNING : Next experiment uses a"
                        + self.experiments[self.activeExperiment].input_device,
                        Colors.BLACK,
                        (self.width / 2, self.height / 2 - 30),
                    )
                ...
                self.write_box(
                    "Press SPACE (or a key of the controller) to continue.",
                    Colors.BLACK,
                    (self.width / 2, self.height / 2 + 30),
                )
            elif self.language == "fr":
                ...
                if self.experiments[self.activeExperiment].input_device != None:
                    self.write_box(
                        " ATTENTION : La prochaine expérience utilise un(e)"
                        + self.experiments[self.activeExperiment].input_device,
                        Colors.BLACK,
                        (self.width / 2, self.height / 2 - 30),
                    )
                    ...
                    self.write_box(
                        " Prêt ? ", Colors.BLACK, (self.width / 2, self.height / 2 - 30)
                    )
                self.write_box(
                    "Appuyez sur ESPACE (ou une touche de la manette) pour continuer.",
                    Colors.BLACK,
                    (self.width / 2, self.height / 2 + 30),
                )
            print("U")
    ...
    def endOfExperiment(self):
        ...
        while running:
            ...
            if self.language == "en":
                ...
                self.write_box(
                    "Thanks for helping us participating at this experiment !",
                    Colors.BLACK,
                    (self.width / 2, self.height / 2),
                )
                self.write_box(
                    "You can now close this window or press ESCAPE.",
                    Colors.BLACK,
                    (self.width / 2, self.height / 2 + 60),
                )
            elif self.language == "fr":
                ...
                self.write_box(
                    "Vous pouvez maintenant fermer cette fenêtre en appuyant sur ECHAP.",
                    Colors.BLACK,
                    (self.width / 2, self.height / 2 + 60),
                )
            ev = pygame.event.get()
    ...
    def quitApp(self):
        ...
        self.save_data_in_file(
            "../users_data/user_" + str(self.experiments_data["user_id"]) + ".json"
        )
        self.running = False
        pygame.display.quit()
        pygame.quit()
    ...
    def pauseMenu(self, current_mode):
        ...
        if self.language == "en":
            self.write_box(
                "PAUSE", Colors.BLACK, (self.width / 2, self.height / 2 - 30)
            )
            self.write_box(
                "Press ESCAPE or SPACE to continue",
                Colors.BLACK,
                (self.width / 2, self.height / 2 + 30),
            )
        elif self.language == "fr":
            ...
            self.write_box(
                "Appuyez sur ECHAP ou ESPACE pour continuer",
                Colors.BLACK,
                (self.width / 2, self.height / 2 + 30),
            )
    ...
    def start(self):
        ...
        while self.running:
            try:
                ...
                self.save_data_in_file(
                    "../users_data/user_"
                    + str(self.experiments_data["user_id"])
                    + ".json"
                )
                exit()
