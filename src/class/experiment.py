...
class Experiment:
    def __init__(
        ...
        self.data = (
            dict()
        )  # contains all user's data ,for this one experiment, about mouse tracking, time, etc...
    ...
    def iterateData(self, game):
        ...
        if isinstance(target, CibleRect):
            ...
            self.data["trials"][self.trial_id]["dimension"] = (
                target.width,
                target.height,
            )
        else:
    ...
    def init_begin(self, game):
        # Nothing here, used for child of Experiment
        # It is called when starting the experiment
