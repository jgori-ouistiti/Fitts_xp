...
class CircleRandomExp(Experiment):
    ...
    def __init__(
        ...
        if isinstance(target_radius, list):
            ...
            if len(self.target_info["radius"]) <= 1:
                raise Exception(
                    "Error: Radius for targets is set as a list but there isn't 2 or more elements in the list. "
                    + "If you want to use one generic value, use just an int an not a list."
                )
        # Distance
        if isinstance(distance, list):
            self.target_info["isDistanceList"] = (
                True  # Boolean = is it a list ? (True),
            )
            self.target_info["indexDistance"] = 0  # index of the radius of next target
            if len(self.target_info["distance"]) <= 1:
                raise Exception(
                    "Error: Distance for targets is set as a list but there isn't 2 or more elements in the list. "
                    + "If you want to use one generic value, use just an int an not a list."
                )
        # ---------
    ...
    def correct_clic(self, game):
        ...
        if self.target_info["isDistanceList"]:
            new_x = int(
                x
                + self.target_info["distance"][self.target_info["indexDistance"]]
                * math.cos(theta)
            )
            new_y = int(
                y
        ...
        if self.target_info["isRadiusList"]:
            marge = (
                self.buffer
                + self.target_info["radius"][self.target_info["indexRadius"]]
            )
        else:
        ...
        while (
            new_x < marge
            or new_x > self.width - marge
            or new_y < marge
            or new_y > self.height - marge
        ):
            ...
            if cpt > 20:
                raise Exception(
                    "Error , could not place the next target as it's always out of the screen. Check width, height and distance."
                )
        ...
        if self.target_info["isRadiusList"]:
            ...
            if self.target_info["indexRadius"] == len(
                self.target_info["radius"]
            ):  # looping index back to zero
        ...
        if self.target_info["isDistanceList"]:
            ...
            if self.target_info["indexDistance"] == len(
                self.target_info["distance"]
            ):  # looping index back to zero
...
class TwoTargetsExp(Experiment):
    ...
    def __init__(
        ...
        if self.target_info["isDistanceList"]:
            ...
            x1 = int(
                width / 2
                - (self.target_info["distance"][self.target_info["indexDistance"]] / 2)
                * math.cos(rad)
            )
            y1 = int(
                height / 2
        ...
        if self.target_info["isRadiusList"]:
            ...
            self.targets.append(
                Cible(
                    (x1, y1), self.target_info["radius"][0], target_color, isTarget=True
                )
            )
            self.targets.append(
                Cible(
                    (x2, y2),
                    self.target_info["radius"][1],
                    target_color,
                    isTarget=False,
                )
            )
            ...
            self.targets.append(
                Cible((x1, y1), self.target_info["radius"], target_color, isTarget=True)
            )
            self.targets.append(
                Cible(
                    (x2, y2), self.target_info["radius"], target_color, isTarget=False
                )
            )
    ...
    def swap_target(self, game):
        ...
        if self.target_info["isDistanceList"]:
            ...
            if self.target_info["indexDistance"] == len(
                ...
                self.target_info["indexDistance"] = 0
            # target 1
            self.targets[0].x = int(
                self.data["width"] / 2
                - (self.target_info["distance"][self.target_info["indexDistance"]] / 2)
                * math.cos(self.data["radian of axe"])
            )
            self.targets[0].y = int(
                self.data["height"] / 2
                - (self.target_info["distance"][self.target_info["indexDistance"]] / 2)
                * math.sin(self.data["radian of axe"])
            )
            # target 2
            self.targets[1].x = int(
                self.data["width"] / 2
                + (self.target_info["distance"][self.target_info["indexDistance"]] / 2)
                * math.cos(self.data["radian of axe"])
            )
            self.targets[1].y = int(
                self.data["height"] / 2
                + (self.target_info["distance"][self.target_info["indexDistance"]] / 2)
                * math.sin(self.data["radian of axe"])
            )
        ...
        if self.targets[0] is self.actual_target:
            ...
            if self.target_info["isRadiusList"]:
                self.targets[0].r = self.target_info["radius"][
                    self.target_info["indexRadius"]
                ]
            ...
            if self.target_info["isRadiusList"]:
                self.targets[1].r = self.target_info["radius"][
                    self.target_info["indexRadius"]
                ]
            self.actual_target = self.targets[0]
            game.active_target = self.targets[0]
            ...
            for event in ev:
                ...
                if event.type == pygame.KEYDOWN:
                    if event.key == pygame.K_ESCAPE:
                        ...
                        if (
                            game.menu("pause") == -1
                        ):  # quitting app because user closed game during pause menu
                            print("QUITTING GAME")
                            game.menu("quit", data=self.data)
                            return game.quitApp()
    ...
    def __init__(
        ...
        for i in range(nb_target):
            ...
            if i % 2 == 0:
                ...
                y = int(
                    height / 2 + self.data["radius of the circle"] * math.sin(theta)
                )
            else:
                x = int(
                    width / 2
                    + self.data["radius of the circle"] * math.cos(math.pi + theta)
                )
                y = int(
                    height / 2
                    + self.data["radius of the circle"] * math.sin(math.pi + theta)
                )
            if i == 0:
                self.targets.append(
                    Cible((x, y), target_radius, target_color, isTarget=True)
                )
            else:
                self.targets.append(
                    Cible((x, y), target_radius, target_color, isTarget=False)
                )
            theta += delta_theta
        self.actual_target = (
            self.targets[0],
            0,
        )  # target object + id of the object in the list
        ...
        while running and self.trial_id < self.maxTrials:
            ...
            for event in ev:
                ...
                if (("cible", True) in L) or (
                    ...
                    and not self.force_target_clic
                ):  # On a cliqué sur une cible
