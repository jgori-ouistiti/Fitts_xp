    ...
    for line in lines:
        if line.startswith("pause"):
            ...
            if type_experience == "url":
                ...
                targets = webEx.getTargetsFromUrl(
                    exp_name, width, height, color=Colors.BLACK, displayInfo=True
                )
                nbMouv = int(parameters[3].strip("\n"))
                if isRandom:
                    list_random_experiences.append(experiment)
                else:
                    list_experiences.append(
                        Experiment(
                            targets,
                            exp_name,
                            exp_id,
                ...
                disposition = parameters[
                    2
                ]  # diposition c'est soit "cercle" soit "densite"
                ...
                if disposition == "cercle":
                    ...
                    if type_cible == "cercle":
                        rayon_D = int(
                            parameters[4]
                        )  # correspond au rayon pour la disposition en cercle
                        rayon_cercle = int(
                            parameters[5]
                        )  # correspond au rayon d'une cible
                        ...
                        if pos[0] == "(":
                            ...
                            pos = pos.split(",")
                            pos = (
                                int(100 / int(pos[0]) * width),
                                int(100 / int(pos[1]) * height),
                            )
                            i = 8
                        else:
                            pos = (width / 2, height / 2)
                        targets = make_circle_target_list(
                            pos, rayon_D, nbCible, Colors.GREEN, rayon_cercle
                        )
                        ...
                        if isRandom:
                            ...
                            list_experiences.append(
                                Experiment(
                                    targets, type_cible, exp_id, nbMouv, noPause=noPause
                                )
                            )
                ...
                ## Disposition est en densite
                    ...
                    center = center.split(",")
                    center = (
                        int(100 / int(center[0]) * width),
                        int(100 / int(center[1]) * height),
                    )
                    ...
                    targets = make_2D_distractor_target_list(
                        dimensions, center, ID, A, p, Colors.BLACK, jmax
                    )
                ...
                experiment = CircleRandomExp(
                    width,
                    height,
                    "Random with W = " + str(radius) + ", distance = " + str(distance),
                    exp_id=exp_id,
                    maxTrials=nb_mouvement,
                ...
                if nbCible % 2 == 0:
                    nbCible += 1
                experiment = CircleExp(
                    width,
                    height,
                    "Circle experiment with W = "
                    + str(radius)
                    + ", distance = "
                    + str(distance)
                    + ", nb_of_target = "
